...
Arch 的话，普通情况安装 `community/clash` 即可。但是要做旁路网关，如果还想代理本机流量的话，需要在之后配置防火墙的时候来区分 clash 流量，防止代理软件的流量本地回环。区分的时候可以用 `uid` / `gid` / `cgroup`。此外如果想要在本地 53 端口直接开 clash 的 DNS、或使用 TPROXY 方法代理的话，需要给予其对应的[“能力”](https://wiki.archlinux.org/title/Capabilities)：

1. `CAP_NET_ADMIN` 让应用可以处理 TPROXY 带来的流量。
...
参考：<https://dev.to/outloudvi/transparent-proxy-on-arch-linux-with-iptables-and-systemd-slice-580c>
...
```bash
sudo systemctl edit clash@jay.service
```
...
如用 cgroup 方案，clash 的配置目录在 `$XDG_CONFIG_HOME/clash/`。需要生成一个软链接指向安装了的数据库文件：
...
> [据说](https://xtls.github.io/documents/level-2/tproxy/#%E5%BC%80%E5%A7%8B%E4%B9%8B%E5%89%8D)这一步如果只是做局域网旁路网关，反而应该把内核数据包转发关掉。不过我没有测试，也不太需要。
...
如果不能，请留意网关机器是不是开了 iptables / nftables 防火墙。iptables 的 filter 表的 FORWARD 链的策略是否是 DROP。如果是这个情况，把策略改成 ACCEPT 再试试：
...
我们先用 `redir-host` 模式而非 `fake-ip` 模式。顾名思义 `fake-ip` 模式下 DNS 服务器会返回一个假的保留 IP 段的 IP，让客户端可以立即发起 TCP 连接而不必等待 Clash DNS 解析，降低响应延迟。但如果单独测试 DNS 功能，返回了假的 IP 显然是连不上的。并且 `fake-ip` 模式下有一些软件或服务可能工作不正常。比如网络连通性测试和时间同步服务等。
...
现在可以把本机网络连接使用的 DNS 服务器改成 Clash 自己的 `127.0.0.1`，然后把之前设置的系统 / 浏览器的 http/socks 代理关掉了。试验一下应该是可以上国内国外网络的。只是此时的 DNS 解析会比 `fake-ip` 慢一些。

### cgroup 方案下放行其他软件

如果是 systemd 服务则参考之前放行 clash 的方法，将服务的 cgroup 设置为 `noproxy.slice`。

如果不用 systemd 服务，可以用一下方式来以普通用户身份按需启动不希望被代理的进程（以 `qbittorrent` 为例）：
...
之后按照此进程的 cgroup 路径配置防火墙试一下，其中的 `1000` 改为你的用户 UID（`$ id`)：

```bash
sudo iptables -t mangle -A clash-self -m cgroup --path "user.slice/user-1000.slice/user@1000.service/noproxy.slice" -j ACCEPT
```

## Fake ip

> cgroup 方案下其他的跳过代理的软件的还要用 clash dns。没有什么很好的办法。除非用 v2ray。 v2ray dns 的实现方法是让你 redirect dns 请求到 v2ray，然后 v2ray 进行 dns 劫持。这样你就可以用 netfilter 控制什么进程的 dns 请求不走 clash dns。
>
> 我想，也许使用更复杂的本机 dns 服务软件，配合 clash dns，应该也可以做到 dns 请求分流。不过我懒得弄了，所以我先用的 redir-host。
...
另外，火狐等浏览器在用户在地址栏中输入了简单的内容并回车的时候也会先进行搜索，同时也进行 DNS 解析。如果 DNS 解析得到了结果（此处是 fake ip），便会提出提示“是否要跳转到 XXX 网址”。在用 fake-ip 的时候就很麻烦，所以建议把这种单词也用 `"*"` 白名单掉。
...
Linux 路由表和 Netfilter 策略路由默认情况下关机后会遗忘。如果配置过程中出了拿不准的错误无法上网了，可以直接重启电脑恢复。如果确定没问题了，可以将路由表配置命令用某种方法在网络服务上线时自动执行，将 Netfilter 配置写入到防火墙服务启动时执行的配置文件中。
...
[Service]
Type=oneshot
ExecStart=ip rule add priority 2333 fwmark 1 table 100 && ip route add local default dev lo table 100
ExecStop=ip rule del table 100 && ip route flush table 100
RemainAfterExit=yes
...
可以用 `systemctl cat iptables`（如果用的 nftables 就看 nftables.service）看一下自己的 systemd 服务在启动时会读哪个文件，然后 `sudo iptables-save -f` 保存到那个文件上去。下次开机就会读取那个文件里的配置了。不过为了干净俺喜欢自己手动改一下。

#### iptables 配置

以下是 cgroup 方案的，clash-user 方案请自行参照修改 `-m` 选项。
...
貌似 clash 是不能通过 Netfilter 的 REDIRECT 方法劫持 DNS 请求的。俺是直接修改 `/etc/resolv.conf` 来强制使用本地 Clash DNS 的：

```bash
nameserver 127.0.0.1
```

NetworkManager 会在连接网络的时候修改这个文件。为了让每个网络都能用上本地的 DNS，俺听取 Arch 群友的建议给这个文件加上了[只读的属性](https://wiki.archlinux.org/title/File_permissions_and_attributes#File_attributes)：
