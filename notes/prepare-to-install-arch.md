...
作为计划投放在个人博客上的第一篇文章，笔者希望能在其中记录下之前在台式机和笔记本电脑上安装Arch Linux的过程。俺~~可怜又可爱~~的小笔记本电脑从高中二年级开始就在稳健地发挥着热量，直到大学毕业准备工作仍未下岗。其硬盘、CPU、内存难以满足当下一套Windows 10 + Firefox Quantum + Electron Apps(Visual Studio Code / Trello)的欲求。恰巧最近想要在Linux环境下学习一些别的框架，挑挑拣拣便看中了这个配置相对复杂但是内容精简的Arch Linux往笔电上试着装了装。
...
- Nvidia不认真支持其显卡在Linux下的使用体验，特别是多显卡交火、双显卡切换等在Linux下的发挥可能不如Windows（但对笔者这种看网页、VSCode打打代码的菜鸡来说感觉不到，除非俺也有四路泰坦。）

总体而言，其实仅就资源占用这一点，在笔者的小笔记本上，Linux就已经一刀处决Windows 10了。笔者便在认为利大于弊的时候，趁着有闲，玩玩Linux。
...
本文将描述Arch Linux安装介质的准备工作，后文将分别记录笔者在一台Windows 10台式机和一台已运行Arch Linux的笔电上，安装最小化、有网络连接的Arch Linux，配置可以使用搜索引擎的环境，以及笔者安装自用软件的过程。
...
至此，准备工作已经完成。笔者之后将会继续描述[在Hyper-V虚拟机上](create-vm-for-arch.md)和[在笔电上](install-arch-on-laptop-and-vm.md)安装Arch Linux的过程，详情请移步后文。
